...
import wasmfile from "./sqlite3.wasm.ts";
import type { VFS } from "./types.ts";

let vfsRef = [] as unknown as [VFS];

function vfs() {
  return vfsRef[0];
}

export function init(newvfs: VFS) {
  vfsRef[0] = newvfs;
}
...
const mod = await WebAssembly.instantiate(wasmfile, {
  host: {
    ...
    fs_close: (fd: number) => vfs().close(fd),
    fs_delete(filename: number) {
      vfs().unlink(str(filename));
    },
    fs_read(
      fd: number,
      buffer: number,
    ...
    fs_truncate: (fd: number, length: number) => vfs().truncate(fd, length),
    fs_filesize: (fd: number) => vfs().filesize(fd),
  },
  session: {
